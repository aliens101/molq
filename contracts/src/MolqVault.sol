...
contract MolqVault is ERC4626, Ownable2Step, Pausable, ReentrancyGuard {
    ...
    function _approveAsset(IERC20 token, address spender, uint256 amount) private {
        (bool success, bytes memory result) = address(token).call(abi.encodeCall(IERC20.approve, (spender, amount)));
